...
void write_metadata(dataTYPE& metadata) {
    ...
    if (ready != "Y" && ready != "y") {
        throw std::runtime_error("Read the instructions again if not ready or ask customer support");
    }
    ...
    while (!done) {
        ...
        if (add_new != "Y" && add_new != "y") {
            std::cout << "Metadata setup is finished" << std::endl;
            done = true;
            continue;
        }
...
void rename_parent_dir(int argc, char** argv) {
    ...
    try {
        ...
        std::cout << "Renamed “" << old_dir.filename().string() << "” to “" << argv[1] << "”\n";
    } catch (const fs::filesystem_error& e) {
        throw std::ios_base::failure(std::string("Failed to rename ‘") + old_dir.string() + "’ to ‘" + new_dir.string() + "’: " + e.what());
    }
}
